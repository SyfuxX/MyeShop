...
if($_POST)
{
	foreach($_SESSION['cart'] as $key => $value) {
		...
		$result = $con -> query("SELECT stock FROM product WHERE id_product = $key");
	...
	{ 
		...
		$result = $con->exec("INSERT INTO `order` (id_user, total_price, datetime, status) VALUES ($id_user, $total, NOW(), 'pending')");

		$id_order = $con->lastInsertID(); // return the last id_registered in the DTB
        ...
        foreach ($_SESSION['cart'] as $key => $value) 
        {
			...
			$result = $con->exec("INSERT INTO order_details (id_order, id_product, quantity, price) VALUES ($id_order, $key, $quantity, $price)");

			$result = $con->exec("UPDATE product SET stock = (stock - $quantity) WHERE id_product = $key"); // Update the stock in the product table
		}

		$msg_error .= '<div class="alert alert-success">Congratulations, your order #' . $id_order . ' is confirmed. You will receive a mail with all the informations related to it soon !</div>';
